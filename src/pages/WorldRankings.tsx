        ...
        <div className="container max-w-5xl mx-auto px-4">
          <div className="mb-8">
            <div className="flex items-center gap-3 mb-2">
              ...
              <h1 className="text-3xl md:text-4xl font-display font-bold">Country Intelligence Score</h1>
            </div>
            <p className="text-muted-foreground text-sm">
              Top 20 countries ranked by active AI agents — the CIS Leaderboard
            </p>
          </div>
          ...
          <div className="flex gap-3 overflow-x-auto pb-4 mb-6 scrollbar-hide">
            ...
            <div className="glass-card rounded-xl p-4 flex flex-col items-center gap-1 min-w-[120px]">
              ...
              <span className="text-xl font-display font-bold">{rankings.reduce((s, n) => s + n.agent_count, 0)}</span>
              <span className="text-xs text-muted-foreground">Total Agents</span>
            </div>
            <div className="glass-card rounded-xl p-4 flex flex-col items-center gap-1 min-w-[120px]">
              <TrendingUp className="w-4 h-4 text-accent" />
              <span className="text-xl font-display font-bold">{rankings.filter((n) => n.agent_count > 0).length}</span>
                  ...
                  rankings.map((n, i) => {
                    ...
                    return (
                      <TableRow key={n.code} className="border-border hover:bg-primary/5 transition-colors">
                        ...
                        <TableCell>
                          <Link to={`/country/${n.code}`} className="flex items-center gap-2 hover:text-primary transition-colors">
                        ...
                        <TableCell>
                          <div className="flex items-center gap-2 min-w-[140px]">
                            ...
                            <span className="font-mono text-xs font-bold w-8 text-right">{n.agent_count}</span>
                          </div>
                        </TableCell>
                        <TableCell className="text-right">
                          <span className="font-mono font-semibold text-primary text-sm">{n.cis_score.toFixed(1)}</span>
                        </TableCell>
                      </TableRow>
                    );
                  })
